...
- `shrink-diff` does not set it — plugins/ceh-coding-agent/skills/shrink-diff/SKILL.md:1-19 (frontmatter block has no `disable-model-invocation` key)
- `design-test-cases` does not set it — plugins/ceh-testing/skills/design-test-cases/SKILL.md:1-14 (frontmatter block has no `disable-model-invocation` key)
...
- Total: 140 lines across 7 files. Only the frontmatter (opening 20 lines) of each file was read since the question concerns only frontmatter keys.
