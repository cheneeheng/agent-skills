...
> *3. **TL;DR** — 2–3 sentences capturing thesis and key takeaway*
...
[Tweets 3–N]        — one idea per tweet, numbered ("2/")
[Closing tweet]     — CTA: link, follow, or question
```

- **Hook tweet**: must work standalone — no "A thread on…" filler. Open with the post's most surprising or high-stakes claim; the hook alone should deliver value or genuine curiosity.
- **Numbered tweets** ("2/" style from tweet 2): each must land on its own — rewrite anything that only makes sense via the previous tweet. No same-sentence-split continuations.
- **Length**: ≤ 280 characters each — count carefully on long ones.
- **Count**: 8–15 depending on content; no filler. If the post only supports 6 good tweets, make 6.
- **Closing tweet**: a CTA — link to the post (`[link]` placeholder), a question to spark replies, or a follow invite. Not "That's it!"
...
- **Lead with the insight, not backstory** — not "I've been thinking about X lately".
...
- **Line breaks for scannability**: 2–4 sentence paragraphs, blank line between — LinkedIn collapses walls of text behind "see more".
- **0–2 hashtags**, only genuinely descriptive ones. In doubt: zero.
- **End with a specific question or observation** tied to the post's argument — not a generic "What do you think?"
...
- **Capture the thesis, not the narrative** — state the actual claim, not "This post is about…". One takeaway.
...
After delivering, ask: *"Anything you'd like adjusted — tone, length, a specific tweet, the LinkedIn opener?"* Also offer **one proactive observation** — something you'd refine even if they're satisfied. Common candidates: hook tweet reads as summary rather than standalone punch; LinkedIn opener buries the lead; TL;DR takeaway is abstract; blurb gives away too much.
