...
Prefer backward-compatible additions (new optional fields, new endpoints). Only version when a breaking change cannot be avoided.

When required: use URL prefix (`/v2/resources`), maintain `/v1/` for a documented deprecation period, and record the timeline as an ADR.
