...
| Workflow Builder | `ceh-workflow-builder` | Turn a repetitive multi-step task into a runnable artifact — interview it into a spec, decide one skill vs a gated workflow, declare the schema for every step-to-step handoff, and emit the set into the target repo's `.claude/skills/` |
...
| `ceh-documentation` | Write Examples | `/ceh-documentation:write-examples` | Write new runnable programs under `examples/`: a numbered feature tour for new users and self-contained copy-paste recipes for integrating the project, every file run before it is kept |
...
| `ceh-workflow-builder` | Build Agentic Workflow | `/ceh-workflow-builder:build-agentic-workflow` | Turn a repetitive multi-step task into a reusable artifact — gate that it is fully specified (delegating the interview if not), apply the one-skill-vs-workflow gate, declare a schema for every step-to-step handoff, emit leaf-first into `.claude/skills/` |
| `ceh-workflow-builder` | Interview Workflow Task | `/ceh-workflow-builder:interview-workflow-task` | Ask the nine questions a workflow artifact needs answered — including the two people skip, re-run damage and irreversible steps — and write them to a workflow spec the builder designs from |
