---
name: ceh-blog-writer
description: >
  Use this skill when the user already has raw material — notes, bullet points, an outline, fragments, or a wall of text — and wants to go straight to a draft without an interview. The key signal is material that has NOT yet been formed into prose: bullets, keywords, rough jottings, an outline. Trigger when the user says things like "turn this into a blog post", "write a post from these notes", "draft a blog post from this", "I've got my notes, just write it", "here's my outline, write the post", or any variation where the material is notes or structured fragments and the user wants drafting, not extraction. Do NOT trigger when the user has only a rough topic or idea and needs to be drawn out — use blog-interviewer instead. Do NOT trigger when the user provides a repo or code artefact (with or without accompanying notes) — use blog-interviewer (Phase 0) instead, which reads the repo first and then interviews for the human story. Do NOT trigger when the user shares written prose (sentences and paragraphs forming a post, even rough) and wants it improved — use blog-editor instead.
---
